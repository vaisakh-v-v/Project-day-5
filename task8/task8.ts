...
export class Router{
    pathRegx: Record<string, string> = {};
    routes: Map<string, (params?: object) => HTMLElement | void>;
    constructor(){
        this.routes = new Map();
        this.register("404.html", error);
    }
    register(path: string, componentFn: Function){
        if(path.includes(":")){
            const pathArr = path.split("/");
            const mappedPathArr = pathArr.map((each) => {
                if(!each.includes(":")) return each;
    ...
    init(){
        ...
        window.addEventListener("popstate", (e) =>{
            ...
            if(typeof target.location !== "object") return;
            const location = target.location;
            if (!location) return;
            if (!("pathname" in location)) return;
            if (typeof location.pathname !== "string") return;
